...
static const CamRes *findRes(const char *name) {
  if (!name || !name[0]) return nullptr;
  for (const auto &r : kResTable) {
    if (strcasecmp(name, r.name) == 0) return &r;
  }
  return nullptr;
}

static const CamRes *findResBySize(framesize_t fs) {
  for (const auto &r : kResTable) {
    if (r.size == fs) return &r;
  }
  return &kResTable[1];  // qvga
}

const char *cameraFramesizeName() { return findResBySize(s_framesize)->name; }

/** JPEG 质量：数值越大压缩越狠、帧越小；高分辨率必须抬高以免 FB-OVF */
static int jpegQualityFor(framesize_t fs) {
  if (fs >= FRAMESIZE_SXGA) return 18;
  if (fs >= FRAMESIZE_HD) return 15;
  if (fs >= FRAMESIZE_SVGA) return 12;
  return 10;
}

static bool jpegHasEoi(const uint8_t *buf, size_t len) {
  if (!buf || len < 4) return false;
  return buf[0] == 0xFF && buf[1] == 0xD8 && buf[len - 2] == 0xFF && buf[len - 1] == 0xD9;
}

/** 分辨率切换后丢掉过渡帧，避免半截 JPEG / NO-EOI */
static void flushCameraFrames(int n) {
  cameraReleaseFrame();
  for (int i = 0; i < n; ++i) {
...
static void applyFramesizeAndQuality(framesize_t fs) {
  sensor_t *s = esp_camera_sensor_get();
  if (!s) return;
  if (s->set_framesize) {
    const int rc = s->set_framesize(s, fs);
    if (rc != 0) ESP_LOGW(TAG, "set_framesize rc=%d", rc);
  }
  if (s->set_quality) s->set_quality(s, jpegQualityFor(fs));
}

bool cameraSetFramesizeName(const char *name) {
  const CamRes *r = findRes(name);
  if (!r) return false;
  s_framesize = r->size;
  if (!camOk) return true;
  applyFramesizeAndQuality(s_framesize);
  flushCameraFrames(2);
  ESP_LOGI(TAG, "framesize %s (%dx%d) q=%d", r->name, r->w, r->h, jpegQualityFor(s_framesize));
  return true;
}
...
bool cameraBegin(XL9555 &xl) {
  ...
  if (!hasPsram && s_framesize > FRAMESIZE_QVGA) s_framesize = FRAMESIZE_QQVGA;
  // JPEG FB 按 init 时 frame_size 用 width*height/5 分配，后续 set_framesize 不扩缓冲；
  // 有 PSRAM 时按 UXGA 预分配，才能拍 HD/SXGA，否则 QVGA 只有 ~15KB → FB-OVF。
  cfg.frame_size = hasPsram ? FRAMESIZE_UXGA : s_framesize;
  cfg.jpeg_quality = jpegQualityFor(s_framesize);
  ...
  camOk = true;
  ESP_LOGI(TAG, "ready framesize=%s q=%d (fb alloc @%s)", cameraFramesizeName(),
           jpegQualityFor(s_framesize), hasPsram ? "UXGA" : "working");
  return true;
}
...
bool cameraCaptureJpeg(uint8_t *&buf, size_t &len) {
  ...
  for (int attempt = 0; attempt < 4; ++attempt) {
    ...
    if (!jpegHasEoi(heldFb->buf, heldFb->len)) {
      ESP_LOGW(TAG, "capture incomplete JPEG len=%u attempt=%d", (unsigned)heldFb->len, attempt);
      cameraReleaseFrame();
      continue;
    }
    buf = heldFb->buf;
