...
| 最新实时网表基线 | 2026-07-15：约 102 位号、41 网络；摄像头/SPI 显示/探照灯仍在图中 |
| DRC | 原理图 DRC API 空；**PCB（PCB2_8）尚未布线**，内层若残留 `3.3V` 平面须改为 `3V3` |
...
- 2026-07-15 实时图：**仍含**摄像头、探照灯、SPI 显示相关器件与 `CAM_*` / `LCD_*` / `LED_*` 网络；固件摄像头流 API 尚未实现。
...
| 舵机 T3-T7 | U16 LED11-15 → SERVO_1..5，5V 供电 |
| 电机 U19/U22 | TB6612；MOT1←U23 LED14/15，MOT2←LED13/12，MOT3←LED10/11，MOT4←LED9/8；STBY=XL9555 IO0_5 |
| 探照灯 | U23 LED0/1/2 → LED_1 / LED_2 / LED_ALL |
| PWM 总失能 | U16/U23 OE#=XL9555 IO0_6，默认禁止输出 |
